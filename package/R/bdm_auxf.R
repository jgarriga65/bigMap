...
# ------------------------------------------------------------------------------
# string-pad functions
# ------------------------------------------------------------------------------

padleft <- function(s, len, chr = ' ')
{
  if ((len -nchar(s) -2) > 0)
  {
    pad <- chr
    for (i in seq(len -nchar(s) -2)) pad <- paste(pad, chr, sep = '')
    s <- paste(s, pad)
  }
  return(s)
}

padright <- function(s, len, chr = ' ')
{
  if ((len -nchar(s) -2) > 0)
  {
    pad <- chr
...
dotfrmt <- function(x, decimals = 2)
{
  s <- as.character(round(x, decimals))
  s <- substr(s, 2, nchar(s))
  while(nchar(s) < (decimals +1)) s <- paste(s, '0', sep = '')
  s
}
