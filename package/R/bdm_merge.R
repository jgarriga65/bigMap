...
#' bdm.optk.s2nr(exMap, data = exMap$data, plot.optk = TRUE, ret.optk = FALSE)
...
bdm.optk.s2nr <- function(data, bdm, info = T, plot.optk = T, ret.optk = F, layer = 1)
{
	...
	if (is.null(bdm$wtt[[layer]]$C)) {
		...
		bdm$optk <- s2nr.optk(bdm, raw.data, verbose = info, layer = layer)
...
bdm.merge.s2nr <- function(bdm, data = NULL, k = 10, plot.merge = T, ret.merge = F, info = T, layer = 1, ...){

	if (is.null(data))
		return(message('+++ Error: must define raw data !'))
	raw.data <- as.big.matrix(data)
	bdm$merge <- s2nr.merge(bdm, raw.data, k = k, verbose = info, layer = layer)
