...
ptsne.get <- function(cl, bdm, info)
{
	bdm.list <- lapply(bdm$ppx, function(ppx) {
		...
		m$t$nnSize <- system.time({
			...
			sumP <- sum(unlist(lapply(chk.list, function(thrd) thrd$P)))
			sumP <- sumP *threads /layers *1.0 /layers
			cat('... sumP', sumP, '/', m$nX, formatC(sumP /m$nX, format = 'e', digits = 4, width = 12), '\n')
