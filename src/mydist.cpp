...
void sqDist::row_spDist(size_t i, double* Li)
{
	double Xi[this ->mX];
	double Xj[this ->mX];
	// this ->minL = DBL_MAX;
	for (size_t v = 0; v < this ->mX; v++) Xi[v] = this ->X[i *this ->mX +v];
	for (size_t j = 0; j < this ->nX; j++) {
		for (size_t v = 0; v < this ->mX; v++) Xj[v] = this ->X[j *this ->mX +v];
		Li[j] = spDist(mX, Xi, Xj);
	}
}

void sqDist::row_d2Dist(size_t i, double* Li)
{
	// this ->minL = DBL_MAX;
	for (size_t j = 0; j < this ->mX; j++) {
		Li[j] = std::pow(this ->X[i *this ->mX +j], 2);
	}
}

void sqDist::row_d1Dist(size_t i, double* Li)
{
	double Xi[this ->mX];
	// this ->minL = DBL_MAX;
	for (size_t v = 0; v < this ->mX; v++) Xi[v] = this ->X[i *this ->mX +v];
	for (size_t j = 0; j < this ->nX; j++) {
		Li[j] = .0;
		for(size_t v = 0; v < this ->mX; v++) Li[j] += std::pow(Xi[v] -this ->X[j *this ->mX +v], 2);
	}
}
