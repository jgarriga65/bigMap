...
bdm.bhtsne <- function(dSet.data, dSet.name = NULL, is.distance = F, is.sparse = F, normalize = F, dSet.labels = NULL, ppx = 100, xppx = 3.0, iters = 200, theta = .5, lRateX = 1.0, exgg = 1, threads = 4, infoRate = 25)
{
	...
	m.list <- bdm.rtsne(dSet.data, m, ppx = ppx, xppx = xppx, iters = iters, theta = theta, lRateX = lRateX, exgg = exgg, threads = threads, infoRate = infoRate)
	return(m.list[[2]])
}
...
bdm.rtsne <- function(dSet.data, m, ppx, xppx = 3.0, iters = 100, theta = .5, lRateX = 1.0, exgg = 1, threads = 4, infoRate = 25)
{
	...
	# +++ cluster stop
	cluster.stop(cl)
	m$ptsne <- list(threads = 1, layers = 1, bh.threads = threads, iters = iters, theta = theta, lRateX = lRateX, exgg = exgg)
	m$ptsne$Y <- Y
