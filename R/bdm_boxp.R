...
#'
#' @param merged A logical value. If TRUE (default value) and the \code{!is.null(bdm$merge)} the boxplots depict  the clusters after merging. If \code{merged = FALSE} or \code{is.null(bdm$merge)} the boxplots correspond to the top-level clustering.
#'
...
#'
#' bdm.example()
#' bdm.boxp(exMap, data = exData[, 1:4])
#' bdm.boxp(exMap, data = exData[, 1:4], byVars = TRUE)

bdm.boxp <- function(bdm, data, byVars = F, merged = T, clusters = NULL, layer = 1)
{
	# get data
	if (is.null(data))
