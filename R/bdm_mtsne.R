...
#'
#' @param is.distance Default value is \code{is.distance = FALSE}. TRUE indicates that raw data is a distance matrix.
#'
#' @param is.sparse Default value is \code{is.distance = FALSE}. TRUE indicates that the raw data is a sparse matrix.
#'
#' @param ppx The value of perplexity to compute similarities. Default value is \code{ppx = 100}.
#'
#' @param iters Number of iters/epoch. Default value is \code{iters = 200}.
#'
#' @param theta Accuracy/speed trade-off factor, a value between 0.33 and 0.8. Default value is \code{theta = 0.5}. If \code{theta < 0.33} the algorithm uses the exact computation of the gradient. The closer it is this value to 1 the faster the computation and the coarser the approximation of the gradient.
#'
...
#'
#' @param infoRate Number of epochs to show output information. Default value is \code{infoRate = 25}.
#'
...
bdm.mtsne <- function(dSet.data, is.distance = F, is.sparse = F, ppx = 100, theta = .5, iters = 200, mpi.cl = NULL, threads = 4, infoRate = 25)
{
