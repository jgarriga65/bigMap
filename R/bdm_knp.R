...
#'
#' @param k.max Maximum neighborhood size to check. Default value is \var{k.max = NULL}, i.e. neighborhood sizes are checked up to \var{n-1}.
#'
#' @param sampling Fraction of data points to check for each neighborhood size. Default value is \var{sampling = 0.9}.
#'
...
bdm.knp <- function(data, bdm, k.max = NULL, sampling = 0.9, mpi.cl = NULL, threads = 4)
{
...
bdm.knp.plot <- function(bdm)
{
	if (is.null(names(bdm))) kNP.plot.list(bdm, ppxfrmt = 0)
	else kNP.plot(bdm)
}
