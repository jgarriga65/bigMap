...
#'
#' @param plot.optk Default value is \code{plot.optk = TRUE}. If TRUE, this function plots the S2NR measure versus the number of clusters.
#'
#' @param ret.optk Default value is \code{ret.optk = FALSE}. For large datasets this computation can take a while, so it can be saved. If TRUE, the function returns a \var{bdm} instance with S2NR attached.
#'
#' @param info Default value is \code{info = TRUE}. If TRUE, all merging steps are shown.
#'
...
#' @param bdm A \var{bdm} instance as generated by \code{bdm.init()}.
#'
#' @param k The number of desired clusters. The clustering will be recursively merged until reaching this number of clusters. Default value \code{k = 10}. By setting \code{k < 0} we specify the number of clusters that we want to merge.
#'
#' @param plot.merge Default value is \code{plot.merge = TRUE}. If TRUE, the merged clustering is plotted.
#'
#' @param ret.merge Default value is \code{ret.merge = FALSE}. If TRUE, the function returns a \var{bdm} instance with the merged clustering.
#'
#' @param info Default value is \code{info = FALSE}. If TRUE, all merging steps are shown.
#'
#' @param layer The \var{bdm$ptsne} layer to be used. Default value is \code{layer = 1}.
#'
#' @param ... If \var{plot.merge} is TRUE, you can pass plotting parameters to \code{bdm.wtt.plot()}.
#'
#' @return None if \code{ret.merge = FALSE}. Else, a \var{bdm} instance with the merged clustering (\var{bdm$merge}).
#'
