...
#'
#' @return A \var{bdm} instance with element \var{$dMap}, a matrix with a soft clustering of the grid cells.
#'
...
#' bdm.example()
#' \dontrun{
#' +++ compute the density map of the clustering found by the mapping protocol itself
#' m <- bdm.dMap(bdm.labels(ex$map), ex$map, threads = 4)
#' }

bdm.dMap <- function(data = NULL, bdm, threads = 4, mpi.cl = NULL, layer = 1){
...
#'
#' @param plot.peaks Default value is \code{plot.peaks = TRUE}. If set to TRUE and the up-stream step \code{bdm$wtt()} is computed the density peak of each cluster is depicted.
#'
#' @param labels.cex If \code{plot.peaks = TRUE}, \code{labels.cex} is the size of the labels of the clusters (as set in \code{par()}). By default \code{labels.cex=0.0} and the labels of the clusters are not depicted.
#'
#' @param layer The number of the layer from which the class density maps are computed. Default value is \code{layer = 1}.
#'
#' @return None.
#'
