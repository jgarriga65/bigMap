...
#'
#' @param data A vector of discret covariates or class labels. The covariate values can be of any factorizable type.
#'
...
#'
#' @details \var{bdm.dMap()} computes the join distribution \eqn{P(V=v_{i},C=c_{j})} where \eqn{V={v_{1},\dots,v_{l}}} is the discrete covariate and \eqn{C={c_{1},\dots, c_{g}}} are the grid cells of the paKDE raster. That is, this function recomputes the paKDE but keeping track of the covariate (or class) label of each data-point. This results in a fuzzy distribution of the covariate (class) at each cell.
#'
...
bdm.dMap <- function(data = NULL, bdm, mpi.cl = NULL, threads = 4, layer = 1){
	...
	if (is.null(data) && !is.null(bdm$wtt[[layer]])) {
		...
		data <- bdm.labels(bdm, layer = layer, merged = !is.null(bdm$merge))
		L <- data
	}
	else {
...
thread.dMap <- function()
{
	...
	else
	{
		...
		for (i in chnk.brks[thread.rank]:(chnk.brks[thread.rank +1] -1))
		{
			...
			z[, which(U == L[i])] <- z[, which(U == L[i])] + pakde$beta[i] * exp(-pakde$beta[i] * sqdst2i)
		}
		return(z)
	}
}
...
#' Class density map plot.
#'
#' @param bdm A \var{bdm} data mapping instance.
#'
#' @param classes A vector with a subset of class names or covariate values. Default value is \var{classes=NULL}, i.e. all classes are plotted.
#'
#' @param join Default value is \var{join = FALSE}, i.e. the class mapping is based on the class conditional distributions. If \var{join = TRUE}, class mapping is based on the overall join distribution.

#' @param class.pltt A colour palette to show class labels in the hard mapping. Default value is \var{class.pltt = NULL}, i.e. a default palette is used.
#'
#' @param pakde.lvls The number of levels of the heat-map when plotting class density maps. Default value is \var{pakde.lvls = 16}.
#'
#' @param pakde.pltt A palette of colours to indicate the levels of the class density maps. The length of the colour palette should be at least the number of levels specified in \var{pakde.lvls}. Default value is \var{pakde.pltt = NULL}, i.e. a default palette is used.
#'
#' @param wtt.lwd The width of the watertrack lines (as set in \var{par()}). Default value is \var{wtt.lwd = 1.0}.
#'
#' @param plot.peaks Default value is \var{plot.peaks = TRUE}, i.e. the density peak of each cluster is depicted.
#'
#' @param labels.cex The size of the clusters labels (as set in \var{par()}). Default value is \var{labels.cex = 1.0}. If \var{labels.cex = 0.0} the labels of the clusters are not depicted.
#'
#' @param layer The ptSNE output layer. Default value is \var{layer = 1}.
#'
#' @return None.
#'
#' @details \var{bdm.dMap.plot()} yields a multi-plot layout where the first plot shows the dominating value of the covariate (or dominating class) in each cell, and the rest of the plots show the density map of each covariate value (or class).
#'
#' The join distribution \eqn{P(V=v_{i},C=c_{j})} is prone to the bias in the marginal distribution of the covariate. Therefore, the join distribution \eqn{P(V=v_{i},C=c_{j})} is transformed, by default, into a conditional distribution \eqn{P(c_{j}|V=v_{i})} (where the \eqn{c_{j}} are the grid cells of the embedding and V is the covariate (or class)). Thus, the first plot shows a hard classification of grid-cells, (cells are coloured based on the dominating value of the covariate (or dominating class), \emph{i.e}. the \eqn{v_{i}} for which \eqn{P(c_{j}|V=v_{i})} is maximum), and the rest of the plots show the conditional distributions \eqn{P(C=c_{j}|V=v_{i})}. This makes the plots of the different classes not directly comparable but the dominant areas of each class can be more easily identified.
#'
#' However, the same plots can be depicted based on the join distribution by setting \var{join = TRUE}. This makes sense when the bias in the covariate values (or classes) is not significant. In this case the hard clustering shows the real dominance of each covariate value (or class) over the embedding area and the density maps are comparable one to each other (although, individually, they are not real density functions as they do not add up to one).
#'
#' The multi-plot layout can be limited to a subset of the values of the covariate (or subset of classes) specified in parameter \var{classes}.
#'
#' @examples
#'
#' # --- load example dataset
#' bdm.example()
#' bdm.dMap.plot(ex$map)
